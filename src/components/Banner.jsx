            ...
            <div className="flex transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${currentSlide * 100}%)` }}>
                {slides.map((slide, index) => (
                    <div key={index} className="w-full flex-shrink-0 relative">
                        <img src={slide.imgSrc} alt={`Slide ${index + 1}`} className="w-full h-full " />
                        <div className={`absolute ${slide.textPosition}`} style={{ top: '50%', transform: 'translate(-50%, -50%)' }}>
                            ...
                            <button className={`p-2 py-1 md:p-3 text-[12px] md:text-lg md:px-10 ${slide.button}`}>Buy Now</button>
                        </div>
                    </div>
                ))}
            </div>
